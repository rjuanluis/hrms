...
class GraphModule:
    ...
    def request_graph(self, *, role, method, url, body, approval_ref):
        self.calls.append((role, method, url, body, approval_ref))
...
class TestAyPEmailBridge(unittest.TestCase):
    ...
    def test_blocks_multiple_candidate_documents(self):
        selected, status = bridge._select_candidate_attachment([self.attachment(), self.attachment("other.docx")])
    ...
    def test_message_pagination_skips_known_page_and_reaches_pending_message(self):
        ...
        responses = iter(
            (
                {
                    "value": [known],
                    "@odata.nextLink": "https://graph.microsoft.com/v1.0/users/empleos/messages?$skiptoken=safe",
                },
                {"value": [pending]},
            )
        )
    ...
    def test_remote_transport_keeps_payload_out_of_process_arguments(self):
        ...
        completed = types.SimpleNamespace(returncode=0, stdout='{"status":"created","applicant":"APP-1"}\n', stderr="")
