...
async function findSession() {
  ...
  for (const { key, value } of db.getRange()) {
    ...
    try {
      ...
      if (typeof record?.id === 'string' && record.id.length > 20) candidates.push(record.id);
    } catch (_) {}
  }
  db.close();
  ...
  for (const session of candidates) {
    try {
      await request(session, '/services/compose/inspectService', { projectName, serviceName });
      return session;
    } catch (_) {}
  }
  return null;
...
(async () => {
  ...
  for (const required of ['configure-workers:', 'queue-documents:', 'bench', 'documents']) {
    if (!candidate.includes(required)) throw new Error(`candidate Compose is missing ${required}`);
  }
  ...
  fs.writeFileSync(path.join(backupDir, 'inspect-service.json'), `${JSON.stringify(before, null, 2)}\n`, { mode: 0o600 });
  ...
  fs.writeFileSync(path.join(backupDir, 'inspect-service-after.json'), `${JSON.stringify(after, null, 2)}\n`, { mode: 0o600 });
