...
Recommended interval: every 5 minutes. Empty stdout means success/no alert. The runner scans message headers in bounded 100-item pages from the 2026-08-13 channel activation boundary, while processing at most the configured per-run limit. Deterministic admission blockers emit one sanitized aggregate JSON object and exit zero so they are not mislabeled as a crashed bridge; infrastructure/runtime failures emit sanitized error JSON and exit non-zero. A vacancy code is optional in the subject: the authenticated server assigns only when `HR-OPN-2026-0001` is the sole open vacancy, while exact current-vacancy consent and all CV security gates remain mandatory. No scheduler is installed by this repository.
