...
if [ "$#" -ne 5 ]; then
  printf 'usage: %s SOURCE_DIR EXPECTED_RUNNER_SHA256 EXPECTED_TEST_SHA256 EXPECTED_LAUNCHER_SHA256 EXPECTED_VACANCY_PARSER_SHA256\n' "$0" >&2
  exit 64
fi
...
vacancy_parser_name=ayp_ats_vacancy_reference.py
runner_source="$source_dir/$runner_name"
test_source="$source_dir/$test_name"
launcher_source="$source_dir/$launcher_name"
vacancy_parser_source="$source_dir/../hrms/recruitment/ats_vacancy_reference.py"
...
install -m 0600 "$vacancy_parser_source" "$tmp_dir/$vacancy_parser_name"
...
install -m 0600 "$vacancy_parser_source" "$install_dir/$vacancy_parser_name"
...
[ "$(sha256 "$install_dir/$vacancy_parser_name")" = "$expected_vacancy_parser_sha" ]

printf '{"status":"installed_not_scheduled","runner_sha256":"%s","test_sha256":"%s","launcher_sha256":"%s","vacancy_parser_sha256":"%s"}\n' \
  "$expected_runner_sha" "$expected_test_sha" "$expected_launcher_sha" "$expected_vacancy_parser_sha"
