...
1. A push to `ayp-production` validates the source and builds an immutable image. The workflow pins the exact HRMS, Frappe, ERPNext and `frappe_docker` commits, the `frappe/build` and `frappe/base` image digests, and a dated Debian package snapshot with explicit document-tool versions; the built image carries and verifies all three application-source markers.
2. GitHub Actions connects using a restricted deployment key. For the first digest migration it deliberately sends the already-supported SHA tag in the forced SSH command and streams the tested digest after the ephemeral registry token. The host requires the tag, digest, checked-out release SHA and in-image HRMS marker to agree before any backup or EasyPanel mutation.
3. `host-deploy-wrapper.sh` permits only an AyP HR SHA-tag deployment, fetches that exact release commit, and passes the independently tested digest to the deployment controller.
4. `deploy.sh` renders the digest into the Compose template, updates that exact immutable source through EasyPanel's authenticated API with a service-scoped rollback artifact, and requests deployment through that same control plane. It verifies every persistent application container plus the one-shot worker configurator against the candidate image ID. A failure before migration restores the prior inline source through the authenticated API, redeploys it, and verifies canonical source, managed file, prior persistent-service count, and the prior image ID of every application service—including prior absence. Legacy rollback also restores the prior local `production` alias when the saved source still uses it. Once migration starts, automatic image downgrade is disabled and the script stops for manual recovery.
5. Only after the digest-based deployment succeeds does GitHub move the mutable `:production` compatibility alias to that deployed digest.
6. The script requires and backs up the existing official site before mutation, waits for all eleven persistent Compose services, verifies the dedicated `documents` queue worker/configuration, migrates the site, applies the idempotent standard AyP configuration and records deployment evidence. A missing or unverifiable production site fails closed; deployment does not bootstrap a replacement.
7. EasyPanel Traefik serves the site at `https://hr.aroypedal.com`.
